            ...
            addAll(
                listOf(
                    ...
                    Theory(
                        ...
                        content = "В Kotlin нулевая безопасность (null safety) — это одна из ключевых особенностей языка, которая предотвращает появление NullPointerException (NPE), распространённой проблемы в Java.\n" +
                                "\n" +
                                "\uD83D\uDD39 Основные принципы null-безопасности:\n" +
                                "Nullable (?) и Non-nullable (!) типы\n" +
                                "\n" +
                                "По умолчанию все переменные non-nullable, то есть не могут быть null:\n" +
                                "var name: String = \"Mikhail\"\n" +
                                "name = null // Ошибка компиляции!\n" +
                                "Чтобы разрешить null, нужно явно указать ?:\n" +
                                "var name: String? = \"Mikhail\"\n" +
                                "name = null // OK\n" +
                                "Оператор безопасного вызова (?.)\n" +
                                "Используется для вызова методов или доступа к свойствам, если объект не null:\n" +
                                "\n" +
                                "val length: Int? = name?.length\n" +
                                "Если name == null, то length тоже будет null, но ошибки не будет.\n" +
                                "\n" +
                                "Оператор элвиса (?:)\n" +
                                "Позволяет задать значение по умолчанию, если переменная null:\n" +
                                "\n" +
                                "val length: Int = name?.length ?: 0\n" +
                                "Если name == null, то length = 0.\n" +
                                "\n" +
                                "Оператор уверенного вызова (!!)\n" +
                                "Используется, когда программист уверен, что переменная не null. Если она всё-таки окажется null, приложение упадёт с NullPointerException:\n" +
                                "\n" +

                                "val length: Int = name!!.length // ⚠\uFE0F Вызовет NPE, если name == null\n" +
                                "Функция let\n" +
                                "Часто используется вместе с ?., чтобы выполнять действия только если переменная не null:\n" +
                                "\n" +
                                "name?.let {\n" +
                                "    println(\"Длина строки: S{it.length}\")\n" +
                                "}\n" +
                                "Если name == null, блок let просто не выполнится.\n" +
                                "\n" +
                                "Типы-платформенные (!)\n" +
                                "Когда код из Java импортируется в Kotlin, его null-тип неизвестен (String!).\n" +
                                "Чтобы избежать NPE, рекомендуется:\n" +
                                "\n" +
                                "Явно указывать тип (String? или String)\n" +
                                "Использовать safe calls (?.)\n" +
                                "Проверять на null перед использованием\n" +
                                "Пример работы с null-значениями в функциях\n" +
